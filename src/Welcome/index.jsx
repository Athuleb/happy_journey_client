...
function TravelLandingPage() {
    ...
    useEffect(() => {
        ...
        if (loading) {
            interval = setInterval(() => {
                setBorderRadius((prev) => (prev === 0 ? 7 : prev - 1));
    ...
    const NavigateLogin = async () => {
        ...
        try {
            // const response = await instance.post("/auth/cache-login/");
            const response = await instance.post("/auth/token-login/", null, {
                headers: { Authorization: token }
            });

            if (response.data.message === 'No cached data found') {
                console.log('No cached data, navigating to manual login');
                navigate('login');
            } else if (response.data.message === 'token response') {

                console.log('token response login successful, navigating to main');
                navigate('/main', {
            ...
            if (error.code === "ERR_NETWORK" || error.message.includes("Network Error")) {
                ...
                console.error('Server response:', error.response.data.response.message);
                navigate('login');
            }
            else {
